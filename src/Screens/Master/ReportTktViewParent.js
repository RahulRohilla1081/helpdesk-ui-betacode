...
function ReportTktView(props) {
  ...
  console.log("ASdajsnbdjhbasdas",thisTicket.TICKET_ID);
  ...
  const ticketApproval = async () => {
    ...
    axios
      ...
      .then((res) => {
        cogoToast.success("Ticket approved");
        getTicketData();
        ...
        consultantMailFormdata.append("CC_EMAIL", thisTicket.CC_EMAIL);
        selectedFiles.forEach((file, index) => {
  ...
  const rejectTicket = () => {
    ...
    axios
      ...
      .then((res) => {
        cogoToast.warn("Ticket rejected");
        if (timeLineData.REMARKS || selectedFiles) {
          let timelinePayload = new FormData();
          timelinePayload.append("TICKET_ID", thisTicket.TICKET_ID);
          timelinePayload.append(
...
console.log("ASdkjnasdhbsad",thisTicket.TICKET_ID);
      axios
  ...
  const HandleEffortsSubmit = async () => {
    ...
    if (
      ...
      axios
        ...
        .then((res) => {
          cogoToast.success("Effort Submitted");
          setEffortsData([]);
          setEffortUnit([]);
          getTicketData();
          if (timeLineData.REMARKS || selectedFiles) {
          ...
          axios
            ...
            .then((resp) => {
              ...
              approverMailFormdata.append("CC_EMAIL", thisTicket.CC_EMAIL);
              selectedFiles.forEach((file, index) => {
              ...
              axios
                ...
                .then((ress) => {
                  console.log("mail sent to approver 2");
                });
            });
        });
    }
  ...
  const trSubmitHandler = () => {
    ...
    axios
      ...
      .then((res) => {
        cogoToast.success("TR Submitted");
        let mailTo = [];
        ...
        TRMovedFormdata.append("CC_EMAIL", thisTicket.CC_EMAIL);
        selectedFiles.forEach((file, index) => {
  ...
  const handlePickup = () => {
    axios
      ...
      .then(() => {
        cogoToast.success("Ticket Picked");
        if (props.LOGGED_IN_DATA.LOGGED_IN_AS == "USER") {
          navigate(DASHBOARD);
        } else if (props.LOGGED_IN_DATA.LOGGED_IN_AS == "EMPLOYEE") {
          navigate(EMPLOYEEDASHBOARD);
  ...
  const handleTimelineSubmit = (TicketStatus, TicketFlag) => {
    let status = "";
    let flag = "";
    console.log("kdjsfsdhsdkds", selectedFiles);
  ...
  const initiateTR = async () => {
    ...
    axios
      ...
      .then((res) => {
        cogoToast.success("TR Initiated");
        getTicketData();
        axios
          .post(AXIOS.defaultPort + AXIOS.mailForTRApproval, {
            TICKET_ID: thisTicket.TICKET_ID,
            CC_EMAIL: thisTicket.CC_EMAIL,
  ...
  const handleTrApproval = async (apprStatus) => {
    ...
    axios
      ...
      .then((res) => {
        cogoToast.success("TR Movement approved");
        getTicketData();
        if (apprStatus == 1) {
          ...
          TRApprMailFormdata.append("CC_EMAIL", thisTicket.CC_EMAIL);
          selectedFiles.forEach((file, index) => {
          ...
          TRReturnMailFormdata.append("CC_EMAIL", thisTicket.CC_EMAIL);
          selectedFiles.forEach((file, index) => {
        ...
        .then((response) => {
          ...
          axios
            ...
            .then((res) => {
              ...
              getTicketData();
              getTimelineData();
              cogoToast.success("Target date submitted");
            });
        })
        .catch((err) => {});
    } else {
            ...
            >
              {(((thisTicket?.ROLE?.includes(2) ||
                ...
                <div className="col-md-4">
                  <Label for="basicpill-email-input4">CC Email</Label>
                  <Input
                    type="text"
                    name="EMAIL"
                    className="form-control"
                  ...
                  <>
                    ...
                    {props.LOGGED_IN_DATA.LOGGED_IN_AS == "EMPLOYEE" &&
                      (thisTicket.ROLE.includes(1) ||
                        thisTicket.ROLE.includes(3)) && (
                        <div className="col-md-4">
                          <Label for="basicpill-email-input4">Assign To</Label>
                          <Select
...
export default connect(mapStateToProps, {})(ReportTktView);
